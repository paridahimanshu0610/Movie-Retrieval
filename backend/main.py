...
def _print_results(
    ...
    if plan.get("sub_queries"):
        for sq in plan["sub_queries"]:
            print(f"  Sub-Q  : [{sq['intent']}] {sq['text'][:width - 14]}")
...
def run_query(query: str, intent: str | None, mode: str, top_k: int) -> None:
    ...
    try:
        ...
        # For multi_aspect queries sub_queries drive the search; for all others
        # use the rewrite (if any) or fall back to the original query text.
